...
namespace Openedu.Week4
{
    ...
    public class QuackMachine
    {
        ...
        private Queue<ushort> _queue = new Queue<ushort>();
            ...
            for (_cursor = 0; _cursor < _program.Length; _cursor++)
            {
                switch (_program[_cursor][0])
                {
                    ...
                    case '%':
                        ...
                        break;
                    case '>':
                        this.GetRegister(_program[_cursor][1]);
                        break;
                    case '<':
                        this.SetRegister(_program[_cursor][1]);
                        break;
                    case 'P':
                    ...
                    case 'J':
                        ...
                        break;
                    case 'Z':
                        this.GoToIfZeroEqual(_cursor);
                        break;
                    case 'E':
                        this.GoToIfEquals(_cursor);
                        break;
                    case 'G':
                        this.GoToIfMoreThan(_cursor);
                        break;
                    case 'Q':
                        this.Exit();
                        break;
                    default:
                        _queue.Enqueue(ushort.Parse(_program[_cursor]));
                        break;
                }
            }
        }
